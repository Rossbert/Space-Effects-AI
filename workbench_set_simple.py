...
delegate = tf.lite.experimental.load_delegate(
    ...
    options = {"layer_name": layer_name,
            "operation_mode" : int(DelegateUtils.OperationMode.convolution),
...
save_file_name = f"delegate_{TFLITE_PATH[-15:-7]}_{DelegateUtils.get_operation_mode(DelegateUtils.OperationMode.convolution)}_{layer_name[:-1]}_{datetime.datetime.now().strftime('%Y-%m-%d')}.csv"
